...
def _dashboard_app(request):
    ...
    if request.user.has_perm('fee.view_fund'):
        models.append({
            'name': 'Funds overview',
    ...
    if request.user.has_perm('fee.view_wallet'):
        models.append({
    ...
    if request.user.has_perm('fee.add_receipt') and request.user.has_perm('fee.add_incometransaction'):
        models.append({
            'name': 'Scan Receipt',
...
def _model_perms(user, model_name):
    add    = user.has_perm(f'fee.add_{model_name}')
    change = user.has_perm(f'fee.change_{model_name}')
    delete = user.has_perm(f'fee.delete_{model_name}')
    view   = user.has_perm(f'fee.view_{model_name}') or change
...
def _transactions_app(request):
    ...
    for name, object_name, model_name, view_only in _TRANSACTIONS_MODELS:
        ...
        models.append({
            ...
            'admin_url': f'/fee/{model_name}/',
            'add_url': f'/fee/{model_name}/add/' if perms['add'] and not view_only else None,
            'view_only': view_only,
            'perms': perms,
        })
    return {
        'name': 'Transactions',
        'app_label': 'fee_transactions',
        'app_url': '/fee/incometransaction/',
