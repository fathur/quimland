...
from ql.fee.models import Report
from ql.fee.services.reports import generate_report_pdf
from ql.fee.services.utils import render_report_markdown
...
class ReportAdmin(admin.ModelAdmin):
    ...
    def generate_pdf_view(self, request, report_id):
        ...
        if report.status == Report.Status.PROCESSING:
            ...
            return redirect('admin:fee_report_change', report.pk)
    ...
    def preview_link(self, obj):
        url = reverse('admin:fee_report_preview', args=[obj.pk])
    ...
    def preview_link_field(self, obj):
        if not obj.pk:
            return '—'
        preview_url = reverse('admin:fee_report_preview', args=[obj.pk])
        pdf_url = reverse('admin:fee_report_generate_pdf', args=[obj.pk])
        return format_html(
